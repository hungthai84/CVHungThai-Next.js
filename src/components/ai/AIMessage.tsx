...
export const AIMessage: React.FC<AIMessageProps> = ({
  ...
  return (
    <div className={`flex gap-2.5 mb-3.5 ${isAI ? 'justify-start' : 'justify-end'}`}>
      {/* Avatar for AI: Circular Image as requested */}
      {isAI && (
        <div className="w-8 h-8 rounded-full overflow-hidden bg-gradient-to-tr from-indigo-600 to-purple-600 border border-indigo-300/40 dark:border-indigo-400/40 shadow-sm shrink-0 mt-0.5 flex items-center justify-center">
          {!imgError ? (
            <img
              src={AI_AVATAR_URL}
              alt="Trí Nhân AI"
      ...
      {/* Avatar for User */}
      {!isAI && (
        <div className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-slate-600 dark:text-slate-300 text-xs shrink-0 mt-0.5">
