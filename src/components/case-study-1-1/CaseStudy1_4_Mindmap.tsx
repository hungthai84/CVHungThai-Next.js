...
import { Network, Sparkles, Cpu, ShieldCheck, Compass, BookOpen, Bot, Layers, Sliders } from "lucide-react";
import { playUiSound } from "../../lib/sound";
import { ProjectCard } from "../../data/projectsData";

export function CaseStudy1_4_Mindmap({ jumpToSection, project }: { jumpToSection: (id: string) => void; project?: ProjectCard }) {
  ...
  const updatePaths = () => {
    ...
    [1, 2, 3, 4].forEach(i => {
      const card = document.getElementById(`pillar-card-1-4-${i}`);
      if (card) {
    ...
    <article id="mindmap-section" className="p-4 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl glass-base space-y-5 sm:space-y-6 transition-all duration-300 mb-8 mt-8">
      <div className="flex items-center space-x-3 border-b border-sky-200/80 dark:border-sky-800/80 pb-3.5 sm:pb-4">
        <h2 className="text-base sm:text-xl md:text-2xl font-bold text-sky-600 dark:text-sky-400 tracking-wide flex items-center gap-2 sm:gap-2.5 flex-wrap">
          ...
          <span>Sơ đồ tư duy 4 trụ cột chuẩn hóa & tự động hóa (SOP & Automation)</span>
        </h2>
      </div>
        ...
        <div className="w-full overflow-x-auto custom-scrollbar touch-pan-x pb-4">
          ...
          <div className="flex justify-center my-12 relative z-10">
            <div ref={centerRef} id="mindmap-center" className="group cursor-pointer p-6 rounded-full bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-700 text-white shadow-2xl shadow-indigo-500/40 text-center w-72 h-72 flex flex-col items-center justify-center border-4 border-white/90 dark:border-slate-700 transform hover:scale-105 transition duration-300 backdrop-blur-2xl ring-8 ring-indigo-400/20">
              ...
              <div className="mt-2 text-[11px] font-bold px-3 py-1 rounded-full bg-white/20 backdrop-blur-md border border-white/30 flex items-center gap-1">
                <ShieldCheck className="w-3 h-3 text-emerald-300" /> Tinh gọn - Tự động - Không độ trễ
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-x-32 gap-y-16 absolute inset-0 z-10 pointer-events-none">
            <div className="pointer-events-auto flex justify-start items-start">
              <div id="pillar-card-1-4-1" onClick={() => { playUiSound("click"); jumpToSection('sec-05'); }} className="w-80 group block p-4 rounded-2xl glass-base bg-gradient-to-br from-sky-500/20 to-sky-400/5 border-2 border-sky-400/60 hover:border-sky-500 hover:shadow-glow-sky cursor-pointer">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h4 className="font-bold text-xs text-sky-700 dark:text-sky-300 group-hover:text-sky-500 transition uppercase flex items-center gap-1">MA TRẬN SOP ĐA KÊNH</h4>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 line-clamp-2 mt-0.5 font-body">150+ kịch bản xử lý nghiệp vụ chuẩn hóa Inbound, Chat, Escalation.</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="pointer-events-auto flex justify-end items-start">
              <div id="pillar-card-1-4-2" onClick={() => { playUiSound("click"); jumpToSection('sec-05'); }} className="w-80 group block p-4 rounded-2xl glass-base bg-gradient-to-br from-purple-500/20 to-purple-400/5 border-2 border-purple-400/60 hover:border-purple-500 hover:shadow-glow-purple cursor-pointer">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h4 className="font-bold text-xs text-purple-700 dark:text-purple-300 group-hover:text-purple-500 transition uppercase flex items-center gap-1">AUTOBOT &amp; SMART TICKETING</h4>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 line-clamp-2 mt-0.5 font-body">Tự động phân loại, gắn tag, gán việc SLA và giải quyết 45% tác vụ lặp.</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="pointer-events-auto flex justify-start items-end">
              <div id="pillar-card-1-4-3" onClick={() => { playUiSound("click"); jumpToSection('sec-05'); }} className="w-80 group block p-4 rounded-2xl glass-base bg-gradient-to-br from-emerald-500/20 to-emerald-400/5 border-2 border-emerald-400/60 hover:border-emerald-500 hover:shadow-glow-emerald cursor-pointer">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h4 className="font-bold text-xs text-emerald-700 dark:text-emerald-300 group-hover:text-emerald-500 transition uppercase flex items-center gap-1">KNOWLEDGE BASE SỐ HÓA</h4>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 line-clamp-2 mt-0.5 font-body">Tra cứu thông minh dưới 5s, đồng bộ 100% chính sách sản phẩm mới.</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="pointer-events-auto flex justify-end items-end">
              <div id="pillar-card-1-4-4" onClick={() => { playUiSound("click"); jumpToSection('sec-05'); }} className="w-80 group block p-4 rounded-2xl glass-base bg-gradient-to-br from-orange-500/20 to-orange-400/5 border-2 border-orange-400/60 hover:border-orange-500 hover:shadow-glow-amber cursor-pointer">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h4 className="font-bold text-xs text-orange-700 dark:text-orange-300 group-hover:text-orange-500 transition uppercase flex items-center gap-1">CẢI TIẾN LIÊN TỤC KAIZEN &amp; PDCA</h4>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 line-clamp-2 mt-0.5 font-body">Review chu kỳ hàng tháng, cắt bỏ thao tác thừa và tối ưu hóa chi phí.</p>
                  </div>
                </div>
              </div>
            </div>
