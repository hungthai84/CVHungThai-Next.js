...
import { Settings, Volume2, Database, Trash2, X, Shield, Sparkles, HelpCircle } from 'lucide-react';
              ...
              <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/60 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-slate-900 dark:text-white">Giọng đọc Tiếng Việt Nam (Gemini TTS / Web Speech)</span>
                  </div>
                ...
                <div className="mt-4">
                  <div className="flex justify-between text-slate-500 mb-1">
                    <span className="text-xs font-semibold">Chọn âm sắc (Gemini Voice):</span>
                  </div>
                  <select
                    value={voiceEngine.getGeminiVoice()}
                    onChange={e => {
                      voiceEngine.setGeminiVoice(e.target.value);
                      // Force a re-render by doing a dummy update if needed, but here it's uncontrolled or we can just call it
                      onUpdateConfig({ }); // dummy to trigger re-render
                    }}
                    className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl text-sm font-medium outline-none focus:border-indigo-500"
                  >
