...
class KnowledgeEngine {
  ...
  public searchKnowledge(query: string, length: AnswerLength = 'medium'): KnowledgeSearchResult {
    ...
    if (
      ...
      return {
        answer: `Trí Nhân trợ lý xin cung cấp thông tin liên hệ: Bạn có thể liên hệ trực tiếp với anh Nguyễn Hùng Thái qua Email: ${this.kb.contact.email} hoặc trang cá nhân LinkedIn. Anh Thái hiện đang sinh sống và làm việc tại ${this.kb.contact.location}.`,
        confidence: 0.9,
        category: 'CONTACT',
        actions: [
    ...
    if (
      ...
      return {
        answer: 'Trí Nhân trợ lý xin gửi đến bạn file CV chính thức của anh Nguyễn Hùng Thái. Bạn có thể xem hoặc tải ngay bằng cách chọn nút bên dưới:',
        confidence: 0.95,
        category: 'CONTACT',
        actions: [
        ...
        suggestions: ['Chuyên môn chính của anh Thái', 'Kinh nghiệm Call Center']
      };
    }
    ...
    if (
      ...
      let text = `Theo dữ liệu hồ sơ, anh Nguyễn Hùng Thái có hơn ${this.kb.profile.years_experience} năm kinh nghiệm thực chiến trong quản lý Call Center & CX. `;
      if (length === 'short') {
        text += `Anh Thái từng đảm nhiệm vai trò ${exps[0].position} tại ${exps[0].company} (quản lý ${exps[0].team_size}).`;
      } else {
        text += `Các vị trí đảm nhiệm tiêu biểu của anh Thái:\n- ${exps[0].position} (${exps[0].period}): Quản lý ${exps[0].team_size}, tối ưu CSAT >94%, FCR >85%.\n- ${exps[1].position} (${exps[1].period}): Đào tạo hơn 1,200 nhân sự nghiệp vụ CSKH.`;
      }
    ...
    return {
      answer: `Trí Nhân trợ lý xin thông tin: Anh Nguyễn Hùng Thái là Chuyên gia Lãnh đạo Chăm sóc Khách hàng (CS Manager) & Trải nghiệm Khách hàng (CX Leader) với hơn 22 năm kinh nghiệm trong quản lý Contact Center, tối ưu hóa quy trình dịch vụ và chuyển đổi số CRM. Bạn muốn tìm hiểu thêm về khía cạnh nào của anh Thái?`,
      confidence: 0.7,
      category: 'PROFILE',
